...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        stack = list(reversed(state.stack[-3:]))
        buffer = list(reversed(state.buffer[-3:]))
        ids = []
        ...
        for i in range(3):
            if i < len(stack):
                ...
                word = "<NULL>"
            ids.append(self.word_vocab[word])
    ...
    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        y = np.zeros(len(self.output_labels), dtype=np.float32)
        y[self.output_labels[output_pair]] = 1.0
        return y
        # return np.zeros(91)
