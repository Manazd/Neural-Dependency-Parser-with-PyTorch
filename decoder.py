    ...
    def parse_sentence(self, words, pos):
        ...
        with torch.no_grad():
            while state.buffer:
                ...
                feats_t = torch.tensor(feats, dtype=torch.long).unsqueeze(0)

                logits = self.model(feats_t)
                probs = torch.softmax(logits, dim=1)
                scores = probs.squeeze(0).cpu().numpy()
                scores = -1 * scores
                ...
                for idx in ranked:
                    ...
                    if action == "left_arc":
                        if (len(state.stack) > 0) and (len(state.buffer) > 0) and (state.stack[-1] != 0):
                        ...
                        if (len(state.buffer) > 0) and not (len(state.buffer) == 1 and len(state.stack) > 0):
                            state.shift()
                            applied = True
                            break
